...
static int alloc_state(ACAuto *ac)
{
...
{
    ACAuto *ac = (ACAuto *)calloc(1, sizeof(ACAuto));
    if (!ac) return NULL;
...
/*  This function is fully thread-safe:
  ...
  ACAuto* without any locks.
 */
ACMatchList ac_scan(const ACAuto *ac, const uint8_t *data, size_t len)
{
    ACMatchList ml;
    ml.count    = 0;
